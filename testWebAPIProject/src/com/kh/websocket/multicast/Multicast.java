 ...
 * configurator : 서버 설정에 관한 부분으로 서버 개통시에 기본적으로 해야 할 설정정보들을 담는 객체
 */
@ServerEndpoint(value="/multicast/{chatRoom}", configurator=GetHttpSessionConfigurator.class)
public class Multicast {
	
	// 각 방 별로 사용자들의 정보를 담기 위한 Map 객체를 선언하여 각 방을 key로, 
	// 사용자 List를 Set<Session> 컬렉션 객체로 담아서 서버 사용자들의 접속정보를
	// 통합 관리하기 위한 객체이다.
    ...
    public void onOpen(EndpointConfig config, Session session, @PathParam("chatRoom") String room) {
    	...
    	// @PathParam : 동적인 url 경로를 통해 접근할 경우 해당 경로 부분을 변수로 
    	//				활용할 수 있게 매개변수로 변경하여 받을 수 있는 어노테이션
    	
    	// session.getUserProperties() : 웹 소켓 세션에 사용자가 별도로 추가하거나 수정할 수 있는
    	//								  설정 부분을 가져올 때 사용하는 메소드
    	
    	// getUserProperties() 해당 설정을 통해 웹 소켓 session 내부에 
     	// 사용자의 ID와 해당 방의 이름을 저장하여 이후에 꺼내어 사용할 수 있게 등록한다.
    ...
    public void onMessage(String message, Session session) throws IOException {
    	...
    	// 어떤 사용자가 보낸 것인지 표현 할 때 사용할 수 있으므로 코드를 작성 해 놓은 부분
    	...
    	userList.stream().forEach( x -> {	// x : Session
    		// 람다식 : 기존 절차 지향언어가 가지는 특성인 익명함수 구현부를 객체 지향언어에서도 흉내내기 위한 소스코드 작성 기술
    		// 쉽게 이해하기 위해 JavaScript의 function(){...} :: (x) -> { ... } 를 생각해보자 !
    		try {
    			// 실제 데이터를 전달하는 로직 부분
    			if(!x.equals(session))
    ...
    public void onClose(Session session) {
       ...
       // 만약 방 안에 사용자가 한명도 없으면 채팅방을 폐쇄한다.
       if(userList.size() == 0)  MulticastViewServlet.roomList.remove(room);	// 사용자가 한명도 없으명 채팅방을 지워버림
    }
